...
- Add keyword-only ``open(..., allow_unverified=False)`` opt-in for the recognized direct-USB LS-40 ED and LS-50 ED identities. The facade, USB product search, and live INQUIRY validation remain fail-closed for unnamed or mismatched hardware, and attempt journals record whether the selected USB identity used the unverified path. LS-4000/8000/2000 and COOLSCANIII remain discovery-only because this worker has no direct-USB identity for them. No LS-40 or LS-50 has been driven by this driver; these paths are hardware-unvalidated.
